...
// Function to fetch 17Lands data for FIN
async function fetch17LandsData() {
  try {
    console.log('Fetching 17Lands data for FIN...');
    const response = await fetch(
      'https://www.17lands.com/card_ratings/data?expansion=fin&format=premierdraft'
    );
    ...
    console.log(`Found ${data.length} cards in FIN set`);
    return data;
  } catch (error) {
...
async function main() {
  try {
    ...
    // Save to JSON file
    const outputPath = path.join(__dirname, 'fin-first-printings.json');
